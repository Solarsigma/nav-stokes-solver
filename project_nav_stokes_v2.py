...
while (np.max(l2_err_norms[curr_iter - 1]) > convergence_tol and curr_iter < max_iter):
    ...
    for j in range(1, ny):
        i = nx
        delta_Qv_xi = (get_Qv(Q[-1][:, i,j]) - get_Qv(Q[-1][:, i-1,j]))
        delta_Qv_eta = 0.25 * (get_Qv(Q[-1][:, i,j+1]) - get_Qv(Q[-1][:, i,j-1])
                                + get_Qv(Q[-1][:, i-1,j+1]) - get_Qv(Q[-1][:, i-1,j-1]))

        Ev_xi_curr[:, i,j] = (
            get_R(s_xi[:, i,j], s_xi[:, i,j], 0.5 * (Q[-1][:, i-1,j] + Q[-1][:, i,j]), 0.5 * (vol[i-1,j] + vol[i,j])) @ delta_Qv_xi
            + get_R(s_xi[:, i,j], 0.5*(s_eta[:, i-1,j]+s_eta[:, i-1,j+1]), 0.5 * (Q[-1][:, i-1,j] + Q[-1][:, i,j]), 0.5 * (vol[i-1,j] + vol[i,j])) @ delta_Qv_eta
        )
        ...
        Ev_xi_curr[:, i,j] = (
            get_R(s_xi[:, i,j], s_xi[:, i,j], 0.5 * (Q[-1][:, i-1,j] + Q[-1][:, i,j]), 0.5 * (vol[i-1,j] + vol[i,j])) @ delta_Qv_xi
            + get_R(s_xi[:, i,j], 0.5*(s_eta[:, i,j]+s_eta[:, i,j+1]), 0.5 * (Q[-1][:, i-1,j] + Q[-1][:, i,j]), 0.5 * (vol[i-1,j] + vol[i,j])) @ delta_Qv_eta
        )
    for i in range(2, nx):
        for j in range(1, ny):
            delta_Qv_xi = (get_Qv(Q[-1][:, i,j]) - get_Qv(Q[-1][:, i-1,j]))
            delta_Qv_eta = 0.25 * (get_Qv(Q[-1][:, i,j+1]) - get_Qv(Q[-1][:, i,j-1])
                                    + get_Qv(Q[-1][:, i-1,j+1]) - get_Qv(Q[-1][:, i-1,j-1]))
            s_eta_curr = 0.25*(s_eta[:, i,j] + s_eta[:, i,j+1] + s_eta[:, i-1,j] + s_eta[:, i-1,j+1])

            Ev_xi_curr[:, i,j] = (
                get_R(s_xi[:, i,j], s_xi[:, i,j], 0.5 * (Q[-1][:, i-1,j] + Q[-1][:, i,j]), 0.5 * (vol[i-1,j] + vol[i,j])) @ delta_Qv_xi
                + get_R(s_xi[:, i,j], s_eta_curr, 0.5 * (Q[-1][:, i-1,j] + Q[-1][:, i,j]), 0.5 * (vol[i-1,j] + vol[i,j])) @ delta_Qv_eta
            )
    ...
    for i in range(1, nx):
        j = ny
        delta_Qv_eta = (get_Qv(Q[-1][:, i,j]) - get_Qv(Q[-1][:, i,j-1]))
        delta_Qv_xi = 0.25 * (get_Qv(Q[-1][:, i+1,j]) - get_Qv(Q[-1][:, i-1,j])
                                + get_Qv(Q[-1][:, i+1,j-1]) - get_Qv(Q[-1][:, i-1,j-1]))
        Fv_eta_curr[:, i,j] = (
            get_R(s_eta[:, i,j], 0.5*(s_xi[:, i,j-1] + s_xi[:, i+1,j-1]), 0.5 * (Q[-1][:, i,j-1] + Q[-1][:, i,j]), 0.5 * (vol[i,j-1] + vol[i,j])) @ delta_Qv_xi
            + get_R(s_eta[:, i,j], s_eta[:, i,j], 0.5 * (Q[-1][:, i,j-1] + Q[-1][:, i,j]), 0.5 * (vol[i,j-1] + vol[i,j])) @ delta_Qv_eta
        )
        ...
        delta_Qv_xi = 0.25 * (get_Qv(Q[-1][:, i+1,j]) - get_Qv(Q[-1][:, i-1,j])
                                + get_Qv(Q[-1][:, i+1,j-1]) - get_Qv(Q[-1][:, i-1,j-1]))
        Fv_eta_curr[:, i,j] = (
            get_R(s_eta[:, i,j], 0.5*(s_xi[:, i,j] + s_xi[:, i+1,j]), 0.5 * (Q[-1][:, i,j-1] + Q[-1][:, i,j]), 0.5 * (vol[i,j-1] + vol[i,j])) @ delta_Qv_xi
            + get_R(s_eta[:, i,j], s_eta[:, i,j], 0.5 * (Q[-1][:, i,j-1] + Q[-1][:, i,j]), 0.5 * (vol[i,j-1] + vol[i,j])) @ delta_Qv_eta
        )
    for i in range(1, nx):
        for j in range(2, ny):
            ...
            delta_Qv_xi = 0.25 * (get_Qv(Q[-1][:, i+1,j]) - get_Qv(Q[-1][:, i-1,j])
                                    + get_Qv(Q[-1][:, i+1,j-1]) - get_Qv(Q[-1][:, i-1,j-1]))
            s_xi_curr = 0.25*(s_xi[:, i,j] + s_xi[:, i+1,j] + s_xi[:, i,j-1] + s_xi[:, i+1,j-1])
            Fv_eta_curr[:, i,j] = (
                get_R(s_eta[:, i,j], s_xi_curr, 0.5 * (Q[-1][:, i,j-1] + Q[-1][:, i,j]), 0.5 * (vol[i,j-1] + vol[i,j])) @ delta_Qv_xi
                + get_R(s_eta[:, i,j], s_eta[:, i,j], 0.5 * (Q[-1][:, i,j-1] + Q[-1][:, i,j]), 0.5 * (vol[i,j-1] + vol[i,j])) @ delta_Qv_eta
            )
    ...
    for i in range(1, nx):
        for j in range(1, ny):
            ...
            nu_cell = get_mu(Q[-1][:, i,j]) / Q[-1][0, i,j]

            delta_tau_cap = min(CFL_MAX / (rho_xi + rho_eta), VNN_MAX * vol[i,j] / (nu_cell * (s_eta_cell**2 + s_xi_cell**2)))

            Q_diff[:, i,j] = - (delta_tau_cap) * ( \
                ((E_xi_curr[:, i+1,j] - Ev_xi_curr[:, i+1,j]) * s_xi[2, i+1,j]
                    - (E_xi_curr[:, i,j] - Ev_xi_curr[:, i,j]) * s_xi[2, i,j])
                + ((F_eta_curr[:, i,j+1] - Fv_eta_curr[:, i,j+1]) * s_eta[2, i,j+1]
                    - (F_eta_curr[:, i,j] - Fv_eta_curr[:, i,j]) * s_eta[2, i,j])
                )
