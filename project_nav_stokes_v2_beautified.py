...
from limiter import FluxLimiterFactory


## CONSTANTS


max_iter = 1000 if ENV == 'prod' else 50
...
grid = Grid2D(filename = GRID_FILENAME)


## SETTING UP Q, E_ksi, F_eta VECTORS

limiter = FluxLimiterFactory().get_limiter(LIMITER)
...
def set_boundary_conditions(Q):
    ...
    ## AIRFOIL_EDGE - no-slip adiabatic
    # rho and et are const
...
while (np.max(l2_err_norms[curr_iter - 1]) > convergence_tol and curr_iter < max_iter):
    ...
    Ev_xi_curr[:, nx,1:ny] = (
        np.matvec(get_R(s_xi[:, nx,1:ny], s_xi[:, nx,1:ny], 0.5 * (Q[-1][:, nx-1,1:ny] + Q[-1][:, nx,1:ny]), 0.5 * (vol[nx-1,1:ny] + vol[nx,1:ny])),
                    delta_Qv_xi,
                    axes=[(0,1),(0,),(0,)])
        + np.matvec(get_R(s_xi[:, nx,1:ny], 0.5*(s_eta[:, nx-1,1:ny]+s_eta[:, nx-1,2:ny+1]), 0.5 * (Q[-1][:, nx-1,1:ny] + Q[-1][:, nx,1:ny]), 0.5 * (vol[nx-1,1:ny] + vol[nx,1:ny])),
                    delta_Qv_eta,
                    axes=[(0,1),(0,),(0,)])
    )
    ...
    Ev_xi_curr[:, 1,1:ny] = (
        np.matvec(get_R(s_xi[:, 1,1:ny], s_xi[:, 1,1:ny], 0.5 * (Q[-1][:, 0,1:ny] + Q[-1][:, 1,1:ny]), 0.5 * (vol[0,1:ny] + vol[1,1:ny])),
                    delta_Qv_xi,
                    axes=[(0,1),(0,),(0,)])
        + np.matvec(get_R(s_xi[:, 1,1:ny], 0.5*(s_eta[:, 1,1:ny]+s_eta[:, 1,2:ny+1]), 0.5 * (Q[-1][:, 0,1:ny] + Q[-1][:, 1,1:ny]), 0.5 * (vol[0,1:ny] + vol[1,1:ny])),
                    delta_Qv_eta,
                    axes=[(0,1),(0,),(0,)])
    )
    ...
    Ev_xi_curr[:, 2:nx,1:ny] = (
        np.matvec(get_R(s_xi[:, 2:nx,1:ny], s_xi[:, 2:nx,1:ny], 0.5 * (Q[-1][:, 1:nx-1,1:ny] + Q[-1][:, 2:nx,1:ny]), 0.5 * (vol[1:nx-1,1:ny] + vol[2:nx,1:ny])),
                    delta_Qv_xi,
                    axes=[(0,1),(0,),(0,)])
        + np.matvec(get_R(s_xi[:, 2:nx,1:ny], s_eta_curr, 0.5 * (Q[-1][:, 1:nx-1,1:ny] + Q[-1][:, 2:nx,1:ny]), 0.5 * (vol[1:nx-1,1:ny] + vol[2:nx,1:ny])),
                    delta_Qv_eta,
                    axes=[(0,1),(0,),(0,)])
    )
    ...
    Fv_eta_curr[:, 1:nx,ny] = (
        np.matvec(get_R(s_eta[:, 1:nx,ny], 0.5*(s_xi[:, 1:nx,ny-1] + s_xi[:, 2:nx+1,ny-1]), 0.5 * (Q[-1][:, 1:nx,ny-1] + Q[-1][:, 1:nx,ny]), 0.5 * (vol[1:nx,ny-1] + vol[1:nx,ny])),
                    delta_Qv_xi,
                    axes=[(0,1),(0,),(0,)])
        + np.matvec(get_R(s_eta[:, 1:nx,ny], s_eta[:, 1:nx,ny], 0.5 * (Q[-1][:, 1:nx,ny-1] + Q[-1][:, 1:nx,ny]), 0.5 * (vol[1:nx,ny-1] + vol[1:nx,ny])),
                    delta_Qv_eta,
                    axes=[(0,1),(0,),(0,)])
    )
    ...
    Fv_eta_curr[:, 1:nx,1] = (
        np.matvec(get_R(s_eta[:, 1:nx,1], 0.5*(s_xi[:, 1:nx,1] + s_xi[:, 2:nx+1,1]), 0.5 * (Q[-1][:, 1:nx,0] + Q[-1][:, 1:nx,1]), 0.5 * (vol[1:nx,0] + vol[1:nx,1])),
                    delta_Qv_xi,
                    axes=[(0,1),(0,),(0,)])
        + np.matvec(get_R(s_eta[:, 1:nx,1], s_eta[:, 1:nx,1], 0.5 * (Q[-1][:, 1:nx,0] + Q[-1][:, 1:nx,1]), 0.5 * (vol[1:nx,0] + vol[1:nx,1])),
                    delta_Qv_eta,
                    axes=[(0,1),(0,),(0,)])
    )
    ...
    Fv_eta_curr[:, 1:nx,2:ny] = (
        np.matvec(get_R(s_eta[:, 1:nx,2:ny], s_xi_curr, 0.5 * (Q[-1][:, 1:nx,1:ny-1] + Q[-1][:, 1:nx,2:ny]), 0.5 * (vol[1:nx,1:ny-1] + vol[1:nx,2:ny])),
                    delta_Qv_xi,
                    axes=[(0,1),(0,),(0,)])
        + np.matvec(get_R(s_eta[:, 1:nx,2:ny], s_eta[:, 1:nx,2:ny], 0.5 * (Q[-1][:, 1:nx,1:ny-1] + Q[-1][:, 1:nx,2:ny]), 0.5 * (vol[1:nx,1:ny-1] + vol[1:nx,2:ny])),
                    delta_Qv_eta,
                    axes=[(0,1),(0,),(0,)])
    )
    ...
    nu_cell = get_mu(Q[-1][:, 1:nx,1:ny]) / Q[-1][0, 1:nx,1:ny]

    delta_tau_cap = np.min((CFL_MAX / (rho_xi + rho_eta), VNN_MAX * vol[1:nx,1:ny] / (nu_cell * (s_eta_cell**2 + s_xi_cell**2))), axis=(0,1))
